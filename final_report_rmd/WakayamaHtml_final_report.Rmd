...
# Q1:FA and interprete{.tabset .tabset-fade .tabset-pills}
## 女性説明変数 : X_f
```{r}
...
d_f_.FA$VAR.rotate %>% data.frame() %>% rownames_to_column() %>% 
  ...
  select(rowname,var_name_Jpn,X1,X2) %>% 
  DT::datatable() %>% DT::formatRound(columns=c('X1', 'X2'), digits=3)
...
LE_d_m_final.reg$model[,-1] %>%DT::datatable()
X_m<-LE_d_m_final.reg$model[,-1]
```
